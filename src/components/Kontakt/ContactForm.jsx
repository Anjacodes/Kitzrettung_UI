...
function ContactForm() {
  ...
  return (
    <form className="text-center flex flex-col text-base items-center w-full my-48" action="https://formspree.io/f/mdojkowl" method="POST" onSubmit={handleSubmit}>
      ...
      <input className="rounded-lg py-2 px-5 mb-4 w-1/2 bg-slate-100" type="text" name="name" placeholder='Your name...' value={name} onChange={event => {setName(event.target.value)}} required />
      <input className="rounded-lg py-2 px-5 mb-4 w-1/2 bg-slate-100" type="email" name="email" placeholder='Your email...' value={email} onChange={event => {setEmail(event.target.value)}} required />
      <textarea className="rounded-lg py-2 px-5 mb-4 w-1/2 bg-slate-100 h-40" name="message" type="textarea" placeholder='Your message...' value={message} onChange={event => {setMessage(event.target.value)}} required />
