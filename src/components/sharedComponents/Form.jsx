...
function Form() {
  ...
  return (
    <form className="text-center flex flex-col text-base align-center lg:mb-4" action="https://formspree.io/f/xknenklz" method="POST" onSubmit={handleSubmit}>
      <input type="text" name="name" placeholder="Name" className="rounded-lg py-2 px-5 mb-2" value={name} onChange={event => {setName(event.target.value)}}/>
      <input type="email" name="email" placeholder="Email" className="py-2 px-5 lg:mb-0 rounded-lg" value={email} onChange={event => {setEmail(event.target.value)}} />
